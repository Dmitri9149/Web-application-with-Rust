...
use crate::handlers::{general::*, home::*};
...
pub fn general_routes(cfg: &mut web::ServiceConfig) {
    cfg.service(web::scope("/general").route("/", web::get().to(general_page_handler)));
}
...
pub fn plant_routes(config: &mut web::ServiceConfig) {
    config.service(
        web::scope("/plants")
            .service(
                web::resource("show/{member_id}/{plant_id}")
            ...
            .service(
                web::resource("update/{member_id}/{plant_id}")
...
pub fn member_routes(config: &mut web::ServiceConfig) {
    config.service(
        web::scope("/members").service(web::resource("/").route(web::get().to(show_members))),
    );
}
